...
- **⭐ Избранное** - отмечайте важные заметки и видите их выделенными в списке
 - **🤖 ИИ-редактор (новое)** - в редакторе доступно меню ИИ с пунктом «Исправить ошибки». Поддерживаются OpenAI/Gemini/Anthropic/OpenRouter; ключи в Настройки → API Keys, выбор провайдера и моделей в «Выбор LLM моделей». Используется строго выбранный провайдер/модель (без фолбэков). Таймаут 180 с.
...
- **Интеграция ИИ** - OkHttp; строгий выбор провайдера и модели (без фолбэков); модели по умолчанию (gpt-5, gemini-2.5-flash, claude-3-7-sonnet-thinking); локальное хранение API ключей; метаданные ИИ на версии (провайдер/модель/время)
