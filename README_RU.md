...
- **⭐ Избранное** - отмечайте важные заметки и видите их выделенными в списке
 - **🤖 ИИ-редактор (новое)** - в редакторе доступно меню ИИ с пунктом «Исправить ошибки». Поддерживаются OpenAI/Gemini/Anthropic/OpenRouter; ключи в Настройки → API Keys, выбор провайдера и моделей в «Выбор LLM моделей». Используется строго выбранный провайдер/модель (без фолбэков). Таймаут 180 с. В «Просмотр версии» подсветка изменений посимвольно (зелёным — добавленное относительно предыдущей; красным — удалённое относительно следующей).
