...
- **⭐ Избранное** - отмечайте важные заметки и видите их выделенными в списке
 - **🤖 ИИ-редактор (новое)** - в редакторе доступно меню ИИ с пунктом «Исправить ошибки». Поддерживаются OpenAI/Gemini/Anthropic/OpenRouter; ключи в Настройки → API Keys, выбор провайдера и моделей в «Выбор LLM моделей». Таймаут сети увеличен до 180 с для длинных текстов.
...
- **Интеграция ИИ** - OkHttp; приоритет провайдеров OpenAI → Gemini → Anthropic; модели по умолчанию (gpt-5, gemini-2.5-flash, claude-3-7-sonnet-thinking); сетевые таймауты 40s; локальное хранение API ключей
